...
public final class Ac4Util {
  ...
  public static Format parseAc4AnnexEFormat(
    ...
    if (bitstreamVersion > 1) {
      ...
      if (dataBitArray.readBit()) { // b_program_id
        shortProgramId = dataBitArray.readBits(16); // short_program_id
    ...
    for (int presentationIdx = 0; presentationIdx < numberOfPresentations; presentationIdx++) {
      ...
      if (presentationVersion > 0) {
        ...
        if (dataBitArray.readBit()) { // b_alternative
          ...
          dataBitArray.readBytes(presentationName, 0, nameLen); // presentation_name
    ...
    for (int id = 0; id < ac4Presentations.size(); id++) {
      ...
      if (ac4Presentation.isChannelCoded) {
        ...
        if (ac4Presentation.numOfUmxObjects > 0) {
          ...
          ac4Presentation.channelCount = ac4Presentation.numOfUmxObjects + lfeChannelCount;
          ...
          if (ac4Presentation.level == 4) {
            ac4Presentation.channelCount =
                ac4Presentation.channelCount == 17 ? 21 : ac4Presentation.channelCount;
          }
        } else {
          ...
          switch (ac4Presentation.level) {
            case 0:
              ...
              ac4Presentation.channelCount = 2;
              break;
            case 1:
              ac4Presentation.channelCount = 6;
              break;
            case 2:
              ac4Presentation.channelCount = 8;
              break;
            case 3:
              ac4Presentation.channelCount = 10;
              break;
            case 4:
              ac4Presentation.channelCount = 12;
              break;
            default:
      ...
      if (ac4Presentation.channelCount <= 0) {
        throw ParserException.createForUnsupportedContainerFeature(
    ...
    Ac4Presentation defaultPresentation = Objects.requireNonNull(ac4Presentations.get(0));
    String codecString = createCodecsString(bitstreamVersion,
        defaultPresentation.version, defaultPresentation.level);

    return new Format.Builder()
        .setId(trackId)
        .setSampleMimeType(MimeTypes.AUDIO_AC4)
        .setChannelCount(defaultPresentation.channelCount)
        .setSampleRate(sampleRate)
        .setAudioPresentations(audioPresentations)
        .setDrmInitData(drmInitData)
        .setLanguage(defaultPresentation.language)
  ...
  private static void parseDsiSubstream(ParsableBitArray data, Ac4Presentation ac4Presentation)
    ...
    if (data.readBit()) { // b_content_type
      ac4Presentation.contentClassifier = data.readBits(3); // content_classifier
  ...
  private static void parseDsiLanguage(ParsableBitArray data, Ac4Presentation ac4Presentation)
    ...
    // Cannot use readBytes() since it is not byte-aligned here.
    data.readBits(languageTagBytes, 0, languageTagBytesNumber * C.BITS_PER_BYTE);
